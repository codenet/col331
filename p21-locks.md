...
interrupts. Recall that `tvinit` marked system call's IDT entries as a "trap
...
wait forever for its buffer `b2` to be read since it is not even in the queue!
Even though P2 "won the race" by writing to `l->qnext` first, it lost its buffer
:-)
