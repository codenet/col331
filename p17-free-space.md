...
> We will see processes exiting in action later when we implement ZOMBIE
processes.
...
remembers the new head of the free list.

With this, `main.c` is now able to start two `initproc` processes by calling
`pinit` twice. `allocproc` in `proc.c` maps the process to a dynamically 
allocated memory instead of using a hardcoded constant `STARTPROC`. Press 
`ctrl+P` after the OS boots to see:
