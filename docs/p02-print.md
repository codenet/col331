...
The uart.c file essentially describes the functions to communicate with the UART interface which in this case is the PC8250A UART(The exact part might be different as different models have been introduced over time, however the basic functionality and register mapping is consistent). 

The PC8250 has 8 programmable registers which are accessed by the processor using port mapped IO. In the case of X86, the location 0x3f8 points to the base register of the UART (Equivalent to the 0 register in the UART datasheet). The pin descriptions given in the datasheet should be read along with the code to understand their function. These registers are written using the outb instructions. The in and out instructions are needed for port mapped IO unlike APICs.
